...
const ADMIN_I18N = {
  pt: {
    ...
    "cms.empty":
      "Nenhum bloco de conteúdo cadastrado para a página selecionada.",
  ...
  en: {
    ...
    "admin.cms.subtitle":
      "Create, edit, and delete Frontoffice text blocks without changing code.",
...
class AdminUI {
  ...
  openMenuModal(id = null) {
    ...
    if (id) {
      ...
      if (ordemInput)
        ordemInput.value = this.menus.findIndex((m) => m.ID_Menu === id) + 1;
  ...
  async saveMenuItem() {
    ...
    const ordem =
      parseInt(document.getElementById("menu-ordem").value, 10) || 0;
  ...
  renderCMSBlocks(pageKey) {
    ...
    listContainer.innerHTML = this.cmsBlocks
      .map((b) => {
        ...
        const keyLabel = b.Block_Key.replace(/_/g, " ").replace(/\b\w/g, (c) =>
          c.toUpperCase(),
        );
